...
%token <ID> LT GT LE GE EQ NE NOT AND OR ATR PLUS_ATR MINUS_ATR TIMES_ATR OVER_ATR PLUS_OP MINUS_OP TIMES_OP OVER_OP
...
lista_decl:
			declaracao {
				Filhos* lista = (Filhos*) malloc(sizeof(Filhos));
				lista[0] = $1;
				$$ = novoNo(1, lista, "DECLARACAO", NULL);
			}
			| declaracao lista_decl {
				Filhos* lista = (Filhos*) malloc(sizeof(Filhos) * 2);
				lista[0] = $1;
				lista[1] = $2;
...
declaracao:
			decl_var {
				Filhos* lista = (Filhos*) malloc(sizeof(Filhos));
				lista[0] = $1;
				$$ = novoNo(1, lista, "VAR_GLOBAL", NULL);
			}
			| decl_func {
				Filhos* lista = (Filhos*) malloc(sizeof(Filhos));
				lista[0] = $1;
				$$ = novoNo(1, lista, "FUNC_GLOBAL", NULL);
			}
			;
...
decl_var:
			tipo_especif var FIM_EXPRESS {
				...
				switch($1->tipo){
					...
					default:
						type = other;
						printf("\t\t### ERRO: [%s] tipo nao encontrado. [%d][%d]\n", $1->valor, $1->linha, $1->coluna);
						break;
				}
...
decl_func:
			tipo_especif nome_func INI_PARAM params FIM_PARAM {
				...
				if(novo != NULL){
					...
					while(temp != NULL){
						item = insere(temp->nome, "", VAR, temp->tipo, 0, NULL);
						if(item == NULL){
							printf("\t\t### ERRO: [%s] parametro repetido. [%d][%d]\n", temp->nome, num_lin, num_char);
						}
						temp = temp->prox;
					}
				}
				...
				switch($1->tipo){
					...
					default:
						val = (char*) malloc(sizeof(char) * (12 + strlen($2->valor)) + 1);
						strcat(val, "Nao-listado ");
						break;
				}
				strcat(val, strdup($2->valor));
				strcat(val, "(params)");
...
decl_local:
			...
			| decl_var decl_local {
				...
				$$ = novoNo(2, lista, "LISTA_DECLS_LOCAIS", NULL);
						...
						if(item2 != NULL){
							if(strcmp(item->valor != NULL ? item->valor : "", "") == 0){
								if(strcmp(item2->valor != NULL ? item2->valor : "", "") != 0){
									...
									//printf("\t\t### ADVERTENCIA: [%s] atribuicao de valor vazio. [%d][%d]\n", $3->valor, $3->linha, $3->coluna);
								}
							}
							else{
								free(item->valor);
...
void printArvore(Node *raiz, int tabs) {
	...
	if (raiz != NULL) {
		...
		if (raiz->valor != NULL) {
			printf("%s%s", raiz->valor, raiz->qtdFi > 0 ? " {\n" : "\n");
		}
